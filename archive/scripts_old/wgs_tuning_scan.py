#!/usr/bin/env python3
"""
WGS 催化因子 f 扫描：f=0.2, 0.15, 0.10, 0.08
...
def run_one(case_name, case, f_val, n_cells=40):
...
def main():
    ...
    for case_name, case in CASES:
        ...
        print(f"\n{case_name} (期望 T={exp.get('TOUT_C')}°C CO={exp.get('YCO')}% H2={exp.get('YH2')}%)")
        print("-" * 70)
        all_results[case_name] = {}
        for f in F_VALUES:
            r = run_one(case_name, case, f)
            all_results[case_name][f"f={f}"] = r
            dT = r["T_C"] - exp.get("TOUT_C", 0)
            dCO = r["CO"] - exp.get("YCO", 0)
            print(f"  f={f:.2f}: T={r['T_C']:.1f}°C (ΔT={dT:+.1f})  CO={r['CO']:.1f}% (ΔCO={dCO:+.1f})  H2={r['H2']:.1f}%  CO2={r['CO2']:.1f}%")
    print("\n" + "=" * 70)
    out = os.path.join(ROOT, "docs", "wgs_tuning_scan_results.json")
