#!/usr/bin/env python3
"""
WGS 平衡常数 K 因子扫描：K_eff = K_eq × WGS_K_Factor
...
K_FACTORS = [1.0, 0.5, 0.3, 0.2, 0.1]  # 1.0=基准，<1 强化逆向 WGS
CASES = [
    ("Paper_Case_6", {
...
def run_one(case_name, case, k_factor, n_cells=40):
...
def main():
    print("WGS 平衡常数 K 因子 (WGS_K_Factor) 扫描")
    print("1.0=基准, <1 使 K 变小 → 强化逆向 WGS → 提高 CO")
    print("=" * 70)
    all_results = {}
    for case_name, case in CASES:
        ...
        for kf in K_FACTORS:
            r = run_one(case_name, case, kf)
            all_results[case_name][f"K_f={kf}"] = r
            dT = r["T_C"] - exp.get("TOUT_C", 0)
            dCO = r["CO"] - exp.get("YCO", 0)
            dCO2 = r["CO2"] - exp.get("YCO2", 0) if exp.get("YCO2") else 0
            print(f"  K_f={kf}: T={r['T_C']:.1f}°C (ΔT={dT:+.1f})  CO={r['CO']:.1f}% (ΔCO={dCO:+.1f})  CO2={r['CO2']:.1f}% (ΔCO2={dCO2:+.1f})  H2={r['H2']:.1f}%")
    print("\n" + "=" * 70)
    out = os.path.join(ROOT, "docs", "wgs_k_scan_results.json")
